import type { Pause } from "../../types/project";
...
export function PauseBar({ index }: { index: number }) {
  ...
  const pause = useProjectStore(
    (store) => store.project.timeline[index],
  ) as Pause;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const w =
        100 *
        Math.min(1, Math.max(0, getCurrentTime() - beginS) / pause.duration);
  ...
  return (
    <div className="relative z-30 flex flex-row w-full border-2">
      <div
        className="-z-10 absolute bg-text opacity-50 h-full"
        ref={progressbarRef}
      ></div>
      <span
        className="cursor-pointer pl-1 text-underline"
        onClick={() => seek(beginS)}
      >
        PAUSE
      </span>
