...
export const useProjectStore = create<ProjectStore>((set) => ({
  ...
  // updates cache
  shiftInTimeline: (index: number, offset: number) =>
    set((state) => {
      ...
      return offset === 0
        ...
        : {
            project: {
              ...state.project,
              timeline: [
                ...state.project.timeline.slice(
                  undefined,
                  Math.min(index, index + offset),
                ),
                ...(offset < 0 ? [state.project.timeline[index]] : []),
                ...state.project.timeline.slice(
                  Math.min(index, index + offset) + (offset > 0 ? 1 : 0),
                  Math.max(index, index + offset) + (offset > 0 ? 1 : 0),
                ),
                ...(offset > 0 ? [state.project.timeline[index]] : []),
                ...state.project.timeline.slice(
                  Math.max(index, index + offset) + 1,
                  undefined,
                ),
              ],
            },
  ...
  newPattern: (clickedMeasureIndex) => {
    ...
    set((state) => {
      const newPatternId = Math.max(
        1 + Math.max(...state.project.patterns.map((p) => p.id)),
        0,
      );
      r = newPatternId;
      return {
        project: {
          ...
          timeline: state.project.timeline.map((m, i) =>
            i === clickedMeasureIndex ? { ...m, pattern: newPatternId } : m,
          ),
        },
      };
    });
  ...
  setMeasurePattern: (measureIndex, patternId) =>
    set((state) => ({
      project: {
        ...state.project,
        timeline: state.project.timeline.map((m, i) =>
          i === measureIndex ? { ...m, pattern: patternId } : m,
        ),
      },
    })),
  ...
  newSection: (at) =>
    set((state) => ({
      project: {
        ...state.project,
        timeline: [
          ...state.project.timeline.slice(undefined, at),
          { type: "section", name: "section" },
          ...state.project.timeline.slice(at, undefined),
        ],
      },
    })),
  ...
  setSectionName: (sectionIndex, name) =>
    set((state) => ({
      project: {
        ...state.project,
        timeline: state.project.timeline.map((s, i) =>
          i === sectionIndex ? { ...s, name: name } : s,
        ),
      },
    })),

  setPauseDuration: (pauseIndex, duration) =>
    set((state) => ({
      project: {
        ...state.project,
        timeline: state.project.timeline.map((s, i) =>
          i === pauseIndex ? { ...s, duration: duration } : s,
        ),
      },
    })),
}));
