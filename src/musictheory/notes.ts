...
export type RelChordName = `${string}${'I' | 'II' | 'III' | 'IV' | 'V' | 'VI' | 'VII'}${string}`;
...
function accidentalsAsString(accidental: number): Accidentals {
    if (accidental === 0)
        ...
        return '#'.repeat(accidental) as Accidentals;
    else
        return 'b'.repeat(-accidental) as Accidentals;
}
...
export function parseAbsChord(chord: AbsChordName): AbsChord {
    ...
    while (qualityFirstCharacter < chord.length && 
        (chord.charAt(qualityFirstCharacter) === 'b' || chord.charAt(qualityFirstCharacter) === '#'))
...
export function parseRelChord(chord: RelChordName): RelChord {
    ...
    while (romanNumeralFirstCharacter < chord.length && 
        (chord.charAt(romanNumeralFirstCharacter) === 'b' || chord.charAt(romanNumeralFirstCharacter) === '#'))
        romanNumeralFirstCharacter++;

    const accidental = romanNumeralFirstCharacter * (chord.charAt(0) === 'b' ? -1 : 1);
    
    let degreeNumber = -1;
    for (let i = 0; i < ROMAN_NUMERALS.length; i++) {
        if (chord.startsWith(ROMAN_NUMERALS[i], romanNumeralFirstCharacter) && (degreeNumber === -1 || ROMAN_NUMERALS[degreeNumber].length <= ROMAN_NUMERALS[i].length)) {
            degreeNumber = i;
        }
    }
    ...
    const quality = chord.slice(romanNumeralFirstCharacter + ROMAN_NUMERALS[degreeNumber].length);
...
export function degreeInKey(note: Note, key: Note): ScaleDegree {
    const degreeNumber = mod(LETTER_INDEX[note.letter] - LETTER_INDEX[key.letter], 7); // zero indexed, increment before showing user
    const accidental = signedSemitoneOffset(notesIntervalOffset(key, note) - MAJOR_SCALE_SEMITONES[degreeNumber]);
...
export function noteInKey(degree: ScaleDegree, key: Note): Note {
    ...
    const letter = Object.keys(LETTER_INDEX).find((l) => LETTER_INDEX[l as AtoG] === letterIndex) as AtoG;
    const accidental = MAJOR_SCALE_SEMITONES[degree.degreeNumber] - mod(LETTER_SEMITONES[letter] - LETTER_SEMITONES[key.letter], 12) + key.accidental + degree.accidental;
